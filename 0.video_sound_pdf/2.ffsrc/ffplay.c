...
//SDL里面定义了main 函数，所以在这里取消sdl 中的 main定义，避免重复定义。
#ifdef CONFIG_WIN32
...
typedef struct VideoState
{
	...
	//音频流指针
	AVStream *audio_st;// 关联的主要数据结构是AVCodecContext 和 AVIStream
	//视频流指针
	AVStream *video_st;
	...
	// 解码后视频图像队列数组
	VideoPicture pictq[VIDEO_PICTURE_QUEUE_SIZE];// 输出视频队列，瘦身后的ffplay只有一项
	...
	// 视频数据包队列同步操作而定义的互斥量指针
	SDL_mutex *video_decoder_mutex;//视频线程互斥
	// 音频数据包队列同步操作而定义的互斥量指针
	SDL_mutex *audio_decoder_mutex;//音频线程互斥
...
{
	AVPacketList *pkt1;
	//先分配一个 AVPacketList结构内存，接着， 140 行从AVPacket 浅复制数据， 141行链表尾置空。
	pkt1 = (AVPacketList *)av_malloc(sizeof(AVPacketList));
	if (!pkt1)
		return  - 1;
	pkt1->pkt =  *pkt;
	pkt1->next = NULL;//原141行

	SDL_LockMutex(q->mutex);
	//往队列中挂接 AVPacketList，并且在150 行统计缓存的媒体数据大小。
	if (!q->last_pkt)
		q->first_pkt = pkt1;
	else
		q->last_pkt->next = pkt1;
	q->last_pkt = pkt1;
	q->size += pkt1->pkt.size;//原150行

	//设置条件量为有信号状态，如果解码线程因等待而睡眠就及时唤醒。
	SDL_CondSignal(q->cond);//原152行

	SDL_UnlockMutex(q->mutex);
	return 0;
}//end packet_queue_put()
...
static int packet_queue_get(PacketQueue *q, AVPacket *pkt, int block)
{
	...
	for (;;)
	{
		...
		if (pkt1)
		{
			//如果队列中有数据，就取第一个数据包，在 191行修正缓存的媒体大小，在 192行浅复制帧/包数据
			q->first_pkt = pkt1->next;
			if (!q->first_pkt)
				q->last_pkt = NULL;
			q->size -= pkt1->pkt.size;//191行
			*pkt = pkt1->pkt;//192行
			//释放掉 AVPacketList结构，此结构在 packet_queue_put()函数中动态分配(137行代码处)。
			av_free(pkt1);
			ret = 1;
			break;
		}
...
//分配 SDL库需要的 Overlay 显示表面，并设置长宽属性。
...
//解码后的视频图像在等待显示间隔时间后，做颜色空间转换，调用 SDL库显示。
//简单认为 cpu耗在前面读文件，解复用，解码的时间为 0，做简单的同步处理逻辑。
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts)
{
	...
	if (vp->bmp)
	{
		...
		pict.linesiz e[0] = vp->bmp->pitches[0];
		pict.linesiz e[1] = vp->bmp->pitches[2];
		pict.linesiz e[2] = vp->bmp->pitches[1];
...
//视频解码线程，主要功能是分配解码帧缓存和 SDL显示缓存后进入解码循环(从队列中取数据帧，解码，计算时钟，显示)，
//释放视频数据帧/数据包缓存。
static int video_thread(void *arg)
{
	...
	for (;;)
	{
		...
		//释放视频数据帧/数据包内存，此数据包内存是在av_get_packet()函数中调用 av_malloc()分配的。
		av_free_packet(pkt);
	}
...
//解码一个音频帧，返回解压的数据大小。特别注意一个音频包可能包含多个音频帧，
//但一次只解码一个音频帧，所以一包可能要多次才能解码完。
//程序首先用 while 语句判断包数据是否全部解完，如果没有就解码当前包中的帧，修改状态参数；
//否则，释放数据包，再从队列中取，记录初始值，再进循环。
...
static int audio_decode_frame(VideoState *is, uint8_t *audio_buf, double *pts_ptr)
{
	...
	for (;;)
	{
		...
		//保存多次解码的中间状态。如果多次解码但不是最后次解码， audio_decode_frame 直接进while 循环
		...
		//程序到这里，可能是初始时 audio_pkt没有赋值；或者一包已经解码完，此时需要释放包数据内存
...
//简单的同步方式，音频按照自己的节拍往前走即可，不需要 synchroniz e_audio()函数同步处理
...
void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
	...
	while (len > 0)
	{
		...
		{
			//如果解码后的数据已全部输出， 就进行音频解码， 并在381 行保存解码数据大小， 在383 行读索引置0。
			...
			else
			{
				...
				is->audio_buf_size = audio_size;//381行
			}
			is->audio_buf_index = 0;//383行
		}
		//385 到391 行，拷贝适当的数据到输出缓存，并修改解码缓存的参数，进下一轮循环。
		//特别注意：由进下一轮循环可知，程序应填满 SDL库给出的输出缓存
...
static int stream_component_open(VideoState *is, int stream_index)
{
	...
	{
		//初始化音频输出参数，并调用 SDL_OpenAudio()设置到SDL 库。
		...
		{
			//wanted_spec 是应用程序设定给 SDL 库的音频参数，
			//spec是 SDL 库返回给应用程序它能支持的音频参数，通常是一致的。
			//如果超过 SDL支持的参数范围，会返回最相近的参数
	...
	switch (enc->codec_type)
	{
		case CODEC_TYPE_AUDIO:
			...
			//初始化音频队列，并在 443行启动广义的音频解码线程。
			memset(&is->audio_pkt, 0, sizeof(is->audio_pkt));
			packet_queue_init(&is->audioq);
			SDL_PauseAudio(0);//443行
			break;
		...
		case CODEC_TYPE_VIDEO:
			...
			//初始化视频队列，并在 452行直接启动视频解码线程。
			packet_queue_init(&is->videoq);
			is->video_tid = SDL_CreateThread(video_thread, is);//452行
			break;
...
//关闭流模块，停止解码线程，释放队列资源。
//通过 packet_queue_abort()函数置abort_request 标志位， 解码线程判别此标志位并安全退出线程
...
static int decode_thread(void *arg)
{
	...
	if (err < 0)
	{
		printf("av_open_input_file() error:%d\n", err);
		ret =  - 1;
		goto fail;
	}
...
//入口函数，初始化 SDL库，注册 SDL消息事件，启动文件解析线程，进入消息循环。
int main(int argc, char **argv)
{
	...
	input_filename = "/root/www/video_sound/5.ffsrc/CLOCKTXT_320.avi";
