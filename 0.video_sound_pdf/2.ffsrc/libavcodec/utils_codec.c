...
//内存动态分配函数，复用 av_malloc()函数，再把分配的内存清0.
...
//编解码库内部使用的缓存区，因为视频图像有 RGB 或YUV 分量格式，所以每个数组有四个分量。
...
//计算各种图像格式要求的图像长宽的字节对齐数，是 1个还是 2个， 4 个， 8个， 16 个字节对齐。
...
//每次取 internal_buffer_count 数据项，用 base[0]来判断是否已分配内存，
//用 data[0]来判断是否已被占用。 base[]和 data[]有多重意义。
//在 avcodec_alloc_context中已把 internal_buffer各项清 0，所以可以用base[0]来判断
int avcodec_default_get_buffer(AVCodecContext *s, AVFrame *pic)
{
	...
	else
	{
		...
		AVPicture picture;
		//计算 CbCr色度分量长宽的与 Y亮度分量长宽的比，最后用移位实现。
		avcodec_get_chroma_sub_sample(s->pix_fmt, &h_chroma_shift, &v_chroma_shift);
		//规整长宽满足特定图像像素格式的要求。
		avcodec_align_dimensions(s, &w, &h);
		//把长宽放大一些， 比如在mpeg4 视频中编码算法中的运动估计要把原始图像做扩展来满足不受限制运
		//动矢量的要求(运动矢量可以超出原始图像边界)
		w+= EDGE_WIDTH*2;
		h+= EDGE_WIDTH*2;
		...
		if (picture.data[2])
			...
			size[2] = 0;
		//注意 base[]和data[]数组还有作为标记的用途， free()时的非NULL判断，这里要清 0。
		...
		for (i = 0; i < 3 && size[i]; i++)
		{
			...
			if (buf->base[i] == NULL)
				return  - 1;
			memset(buf->base[i], 128, size[i]);//128即使0,http://blog.csdn.net/leixiaohua1020/article/details/50534150
			//内存对齐计算。
...
//释放占用的内存数组项。保证从 0到 internal_buffer_count-1数据项为有效数据，其他是空闲数据项
void avcodec_default_release_buffer(AVCodecContext *s, AVFrame *pic)
{
	...
	//把将要空闲的数组项和数组最后一项交换，保证 internal_buffer_count计算正确无误。注意这里并
	//没有内存释放的动作，便于下次复用已分配的内存
	temp =  *buf;
	*buf =  *last;
	*last = temp;
	// 把data[i]置空，指示本块内存没有被占用， 实际分配的首地址保持在base[]中。
	// 整个程序最多分配 INTERNAL_BUFFER_SIZE次 avframe，其他次循环使用
	for (i = 0; i < 3; i++)
	{
		pic->data[i] = NULL;
	}
...
void avcodec_default_free_buffers(AVCodecContext *s)
{
	...
	for (i = 0; i < INTERNAL_BUFFER_SIZE; i++)
	{
		...
		for (j = 0; j < 4; j++)
		{
			//av_freep()函数调用的 av_free()函数做了非NULL 判断， 并且分配时已置NULL， 所以内循环可以到4，
...
int avcodec_open(AVCodecContext *avctx, AVCodec *codec)
{
	...
	{
		//这里体现了 priv_data_siz e 参数的重大作用，如果没有这个参数，
		//就要用 codec 结构的名字比较确定具体编解码器使用的上下文结构大小，超级长的 if-else语句
...
//查找编解码器，在本例中，读 avi 文件头得到 codec FOURCC，再由 FOURCC 查找 codec_bmp_tags
//或 codec_wav_tags得到 CodecID 传给此函数
