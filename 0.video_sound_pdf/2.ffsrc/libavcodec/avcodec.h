...
#define FF_INPUT_BUFFER_PADDING_SIZE 8
	...
	//AVPicture 和AVFrame 主要表示解码过程中的使用缓存，通常帧缓存是 YUV 格式，
	//输出格式有 YUV 也有 RGB 格式，所以定义了4 个data 指针来表示分量
	...
	//程序运行时当前 Codec 使用的上下文，
	//着重于所有 Codec 共有的属性(并且是在程序运行时才能确定其值)，
	...
	typedef struct AVCodecContext
	{
		...
		enum PixelFormat pix_fmt;//输出像素格式/视频像素格式
		...
		struct AVCodec *codec;//指向codec指针
		void *priv_data;// 具体解码器属性，在本例中指向MsrleContext 或 TSContext
	...
	typedef struct AVCodec
	{
		...
		enum CodecType type;// 编解码器类型，有效取值为CODEC_TYPE_VIDEO 或CODEC_TYPE_AUDIO
		enum CodecID id;// 编解码器ID 值
		int priv_data_size;// 具体编解码属性结构的大小，取代很多的if-else 语句
...
//调色板大小和大小宏定义，每个调色板四字节(R,G,B,α)。有很多的视频图像颜色种类比较少，
//用索引间接表示每个像素的颜色值，就可以用调色板和索引值实现简单的大约的 4:1 压缩比
