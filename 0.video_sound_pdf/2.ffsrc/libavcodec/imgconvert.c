...
int avpicture_fill(AVPicture *picture, uint8_t *ptr, int pix_fmt, int width, int height)
{
	...
	//图像像素大小规整，比如YUV420P 宽度和高度必须是2 的整数倍，如果不符合，程序自动填充补足。
...
//初始化 AVPicture 结构。 输入像素格式和长宽， 计算图像大小， 分配图像缓存， 填充AVPicture 结构。
int avpicture_alloc(AVPicture *picture, int pix_fmt, int width, int height)
{
	...
	//填充 AVPicture 结构。
	avpicture_fill(picture, (uint8_t *)ptr, pix_fmt, width, height);
	return 0;
fail:
...
//释放 AVPicture 分配的内存，因为内存首地址在picture->data[0]中，所以可以简单的释放。
void avpicture_free(AVPicture *picture)
{
	av_free(picture->data[0]);
}

//计算各种图像格式平均每个像素占用的 bit位数。
