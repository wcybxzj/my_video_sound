...
int simplest_flv_parser(char *url){
	...
	//根本不需要执行因为, 前一个fread已经读取了9个字节了
	fseek(ifh, reverse_bytes((byte *)&flv.DataOffset, sizeof(flv.DataOffset)), SEEK_SET);

	//process each tag
	do {
		//读4个字节的previoustagsize
		...
		switch (tagheader.TagType) {
			...
			case TAG_TYPE_VIDEO:{
									...
									//video tag 4bit frame type + 4bit encode type
...
{
	simplest_flv_parser("/root/www/video_sound_data/5.flv_data/408.flv");
	return 0;
}
