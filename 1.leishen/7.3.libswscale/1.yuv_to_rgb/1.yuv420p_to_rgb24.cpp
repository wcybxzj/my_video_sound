...
//rgb24视频可以用yuvplayer看
int main(int argc, char* argv[])
{
	//Parameters
	FILE *src_file =fopen("/root/www/video_sound_data/3.h264_data/ds_480x272.yuv", "rb");
	if (src_file==NULL) {
		printf("fopen error\n");
		exit(1);
	}
	...
	//1.通过AVPixFmtDescriptor结构体可以获得不同像素格式的一些信息。
	//例如下文中用到了av_get_bits_per_pixel()，通过该函数可以获得指定像素格式每个像素占用的比特数（Bit Per Pixel）。
	...
	uint8_t *temp_buffer=(uint8_t *)malloc(src_w*src_h*src_bpp/8);//除8的意思:bit->byte
	...
	//Show AVOption
	av_opt_show2(img_convert_ctx,NULL,AV_OPT_FLAG_VIDEO_PARAM,0);
	sleep(3);
	//Set Value
	av_opt_set_int(img_convert_ctx,"sws_flags",SWS_BICUBIC|SWS_PRINT_INFO,0);//设置算法
