...
prot字段关联相应的广义输入文件； 
priv_data 字段关联各个具体广义输入文件的句柄
*/
typedef struct URLContext {
    const AVClass *av_class;    /**< information for av_log(). Set by url_open(). */
    const struct URLProtocol *prot;
    void *priv_data;//fd or socket fd
...
//每种协议（rtp，rtmp，file等）对应一个URLProtocol。
/*
file 的主要结构的初始化如下：
...
URLProtocol是类似COM 接口的数据结构,表示广义的输入文件，着重于功能函数，一种广义的输入文件
对应一个URLProtocol结构，比如file， pipe， tcp等等，但瘦身后的ffplay 只支持file 一种输入文件。
next 变量用于把所有支持的广义的输入文件连接成链表，便于遍历查找。
*/
